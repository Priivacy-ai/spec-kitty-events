...
class TestCanonicalToSyncV1Mapping:
    ...
    def test_canonical_to_sync_v1_specific_mappings(self) -> None:
        ...
        assert CANONICAL_TO_SYNC_V1[Lane.APPROVED] is SyncLaneV1.DONE
...
class TestSyncLaneV2Enum:
    """Tests for the SyncLaneV2 enum."""

    def test_sync_lane_v2_has_exactly_five_members(self) -> None:
        assert len(SyncLaneV2) == 5

    def test_sync_lane_v2_values(self) -> None:
        assert SyncLaneV2.PLANNED.value == "planned"
        assert SyncLaneV2.DOING.value == "doing"
        assert SyncLaneV2.FOR_REVIEW.value == "for_review"
        assert SyncLaneV2.APPROVED.value == "approved"
        assert SyncLaneV2.DONE.value == "done"


class TestCanonicalToSyncV2Mapping:
    """Tests for the CANONICAL_TO_SYNC_V2 mapping constant."""

    def test_canonical_to_sync_v2_mapping_completeness(self) -> None:
        for lane in Lane:
            assert lane in CANONICAL_TO_SYNC_V2, f"Missing key: {lane}"

    def test_canonical_to_sync_v2_specific_mappings(self) -> None:
        assert CANONICAL_TO_SYNC_V2[Lane.PLANNED] is SyncLaneV2.PLANNED
        assert CANONICAL_TO_SYNC_V2[Lane.CLAIMED] is SyncLaneV2.PLANNED
        assert CANONICAL_TO_SYNC_V2[Lane.IN_PROGRESS] is SyncLaneV2.DOING
        assert CANONICAL_TO_SYNC_V2[Lane.FOR_REVIEW] is SyncLaneV2.FOR_REVIEW
        assert CANONICAL_TO_SYNC_V2[Lane.APPROVED] is SyncLaneV2.APPROVED
        assert CANONICAL_TO_SYNC_V2[Lane.DONE] is SyncLaneV2.DONE
        assert CANONICAL_TO_SYNC_V2[Lane.BLOCKED] is SyncLaneV2.DOING
        assert CANONICAL_TO_SYNC_V2[Lane.CANCELED] is SyncLaneV2.PLANNED

    def test_canonical_to_sync_v2_function(self) -> None:
        for lane in Lane:
            assert canonical_to_sync_v2(lane) is CANONICAL_TO_SYNC_V2[lane]
