...
"which payload IDs / event types this package supports": 18 journal rows
(the `STRICT_EVENT_TYPES` members other than `HarnessObservation`) plus 6
observation rows (one per `ObservationKind`, each with its payload ID).
