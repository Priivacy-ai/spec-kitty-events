...
| T009 | Register all seven event types in `_EVENT_TYPE_TO_MODEL` in `src/spec_kitty_events/conformance/validators.py`. | WP02 | |
| T010 | Add `LOCAL_ONLY_EVENT_TYPES: frozenset[str] = frozenset()` to `src/spec_kitty_events/__init__.py` and re-export new payload models. | WP02 | |
| T011 | Add seven canonical fixtures under `src/spec_kitty_events/conformance/fixtures/events/valid/` produced by `model.model_dump(mode="json")`. Register in `manifest.json` with `min_version: "5.2.0"`. | WP02 | [P] |
| T012 | Add `tests/unit/test_seven_event_contracts.py` with round-trip and registry tests per type, plus `LOCAL_ONLY_EVENT_TYPES` properties. | WP02 | |
...
| T016 | Export the legacy surface from `src/spec_kitty_events/__init__.py`. | WP03 | |
| T017 | Add fixtures `conformance/fixtures/legacy/pre_3_0_envelope_normalizes.json` and `conformance/fixtures/legacy/unrecognized_legacy_diagnostic.json`. Register in manifest. | WP03 | [P] |
...
| FR-011 (seven entries in `_EVENT_TYPE_TO_MODEL`) | WP02 |
| FR-012 (seven valid fixtures + manifest entries) | WP02 |
| FR-013 (LOCAL_ONLY_EVENT_TYPES surface) | WP02 |
...
**Risks**: 1) Bootstrap-planned events (forced `*→planned` with `from_lane=None`) must still pass. Mitigated by T004 explicit test. 2) The semantic validator could fire on a payload whose pydantic model layer already rejected the shape, double-faulting. Mitigated by gating the dispatch on `len(model_violations) == 0`. 3) Violation message format must be preserved exactly so downstream consumers continue to route on `force=True` and `review-rejection` substrings. Mitigated by T004 substring assertion.
...
### WP02 — Seven canonical event-type contracts + `LOCAL_ONLY_EVENT_TYPES`

**Goal**: Ship canonical pydantic payload models for the seven SaaS-bound event types currently emitted without contracts. Register them in `_EVENT_TYPE_TO_MODEL`, add canonical fixtures, and publish `LOCAL_ONLY_EVENT_TYPES` (empty) as a stable machine-readable surface.
...
- `src/spec_kitty_events/conformance/validators.py` (extend `_EVENT_TYPE_TO_MODEL` only — does NOT overlap WP01's semantic dispatch change because both edits are to the same file but at different line ranges; the implement-review loop will sequence WP01 → WP02 if needed via lane assignment, or merge cleanly because the edits are textually distant)
- `src/spec_kitty_events/__init__.py` (extend `__all__` and exports)
...
- `src/spec_kitty_events/conformance/fixtures/manifest.json` (extend)
- `tests/unit/test_seven_event_contracts.py` (NEW)

**Authoritative surface**: `src/spec_kitty_events/`.

**Dependencies**: none (parallel-safe with WP01 and WP03).

**Note on shared file `validators.py`**: WP01 modifies `validate_event()` body and adds `_SEMANTIC_VALIDATORS`; WP02 adds seven entries to `_EVENT_TYPE_TO_MODEL` (a separate dict). The edits do not collide textually. Lane assignment via `finalize-tasks` will put WP01 and WP02 in different lanes; the lane merge will combine both edits.

**Risks**: 1) Field shapes might drift from the CLI emitter. Mitigated by basing the models on the pre-mission audit (`spec-kitty/src/specify_cli/sync/emitter.py` lines 720–1431). 2) `extra="forbid"` could reject benign extra fields the producer wants to add later. Mitigated by the spec's explicit decision: drift becomes a hard error at the contract boundary (intended behavior). 3) Fixtures must round-trip through `model.model_dump(mode="json")` to stay canonical-shaped (per C-004). T011 enforces this.

**Included subtasks**:
- [ ] T006 Create `build_lifecycle.py` (WP02)
- [ ] T007 Extend `project_lifecycle.py` (WP02)
- [ ] T008 Extend `lifecycle.py` (WP02)
- [ ] T009 Register seven types in `_EVENT_TYPE_TO_MODEL` (WP02)
- [ ] T010 Export `LOCAL_ONLY_EVENT_TYPES` (WP02)
- [ ] T011 Add seven canonical fixtures + manifest entries (WP02)
- [ ] T012 Add `tests/unit/test_seven_event_contracts.py` (WP02)
...
- `src/spec_kitty_events/__init__.py` (extend exports — concurrent edit with WP02; same note as WP02 applies: separate import lines, lane merge handles it)
...
- `src/spec_kitty_events/conformance/fixtures/manifest.json` (extend — concurrent edit with WP02)
- `tests/unit/test_legacy_normalizer.py` (NEW)

**Authoritative surface**: `src/spec_kitty_events/`.

**Dependencies**: none (parallel-safe with WP01 and WP02).
...
- [ ] T016 Export from `__init__.py` (WP03)
- [ ] T017 Add success and un-normalizable fixtures + manifest entries (WP03)
- [ ] T018 Add `tests/unit/test_legacy_normalizer.py` (WP03)

### WP04 — Pyargs entrypoint wrapper-shape extraction + stale fixture fix

**Goal**: Bring the pyargs conformance entrypoint from 22 failures to zero. Fix the wrapper-shape misroute for class_taxonomy / historical_row / lane_mapping fixtures, and the stale `wp-status-changed-invalid-lane` fixture that treats canonical `in_review` as invalid.
...
- `src/spec_kitty_events/conformance/fixtures/manifest.json` (note line for the fixed fixture — concurrent edit with WP02/WP03; lane merge handles it)

**Authoritative surface**: `src/spec_kitty_events/conformance/`.

**Dependencies**: **WP01** (so the unforced-backward fixtures now fail through `validate_event` via the semantic dispatch), **WP02** (so the seven new fixtures registered in the manifest are recognized when the entrypoint runs).

**Risks**: 1) The lane_mapping fixtures embed `{legacy_lane, canonical_lane}` inside `.input` — they must be skipped, not routed to `validate_event`. T020 ensures this. 2) The fix to the stale `in_review` fixture must produce a value that still fails the existing pydantic Lane enum validation; T021 uses a typo (`in_reveiw`) to guarantee enum rejection. 3) The 22-failure baseline is the pre-mission gold; after this WP all 22 should be resolved.

**Included subtasks**:
- [ ] T019 Detect wrapper-shape fixtures; extract `.input` (WP04)
- [ ] T020 Special-case lane_mapping fixtures (WP04)
