...
**Organization**: 48 subtasks (`T001`–`T048`) roll up into 9 work packages (`WP01`–`WP09`). Each work package is independently deliverable and testable.
...
- [ ] T005 Implement `FocusTarget` frozen Pydantic model (must be hashable for dict keys)
...
- `FocusTarget` must implement `__hash__` and `__eq__` via Pydantic frozen config for use as dict keys in `participants_by_focus`
...
- `ReducedCollaborationState.participants_by_focus` uses `FocusTarget` as dict key (hashable required)
...
**Goal**: Create 7 conformance fixture files (5 valid, 2 invalid), register in manifest.json, and add property tests for reducer determinism plus performance benchmark.
**Independent Test**: All fixtures pass dual-layer validation, property tests prove determinism across 200+ orderings, benchmark runs under 1s for 10K events.
**Prompt**: `tasks/WP08-conformance-fixtures-and-tests.md`
**Estimated prompt size**: ~500 lines

### Included Subtasks
- [ ] T043 Create `conformance/fixtures/collaboration/valid/` directory and 5 valid fixture JSON files (3-participant-overlap, step-collision-llm, decision-with-comments, participant-lifecycle, session-linking)
- [ ] T044 Create `conformance/fixtures/collaboration/invalid/` directory and 2 invalid fixture JSON files (unknown-participant-strict, missing-required-fields)
- [ ] T045 Register all 7 fixtures in `conformance/fixtures/manifest.json` with event_type and min_version
...
### Implementation Notes
- Fixture format: each JSON file contains an array of Event dicts with collaboration payloads
- 3-participant-overlap fixture: 3 humans join with auth_principal_id → active drive intent → shared WP focus → ConcurrentDriverWarning → WarningAcknowledged (continue + hold)
- step-collision-llm: 2 llm_context participants → same WP step → PotentialStepCollisionDetected → completion
- unknown-participant-strict: event from non-rostered participant (expected to fail strict validation)
...
- Benchmark stability: use generous threshold (<1s for 10K events is very conservative for pure CPU)
...
- [ ] T052 Run full test suite (`python3.11 -m pytest`) and mypy (`mypy --strict`) to verify package integrity
...
| T043 | Create 5 valid conformance fixtures | WP08 | P2 | Yes |
| T044 | Create 2 invalid conformance fixtures | WP08 | P2 | Yes |
| T045 | Register fixtures in manifest.json | WP08 | P2 | No |
